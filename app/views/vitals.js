 ...
 * by the Health Auto Export app posting to anvil's Worker, which writes
 * vitals.json in anvil-data (a PWA cannot read HealthKit itself), so an empty
 * file is the normal pre-connection state, not an error.
 *
...
export function VitalsView({ vitals, loading, hasToken }) {
  ...
  return html`
    <div class="view">
      ...
      <p class="hint">
        Read-only mirror of Apple Health. Written by anvil's Worker, not editable here.
      </p>
    </div>
  `;
}
