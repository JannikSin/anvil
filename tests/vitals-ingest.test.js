...
// unit tests above cannot: that /vitals answers a post with no Origin header
// (an iOS app sends none), that the key is actually enforced, and that a
// payload which parses to nothing fails loudly instead of returning 200.
// None of them reach GitHub, so they touch no real data.
import worker from "../worker/src/index.js";

const post = (path, body, headers = {}) =>
  new Request(`https://anvil-worker.example.workers.dev${path}`, {
    method: "POST",
    headers: { "content-type": "application/json", ...headers },
    body: JSON.stringify(body),
  });

const ENV = { VITALS_KEY: "test-key-0123456789", ANVIL_DATA_WRITE_TOKEN: "ghp_fake" };

test("endpoint: a no-Origin post is answered, not blocked", async () => {
  // Health Auto Export is a native app and sends no Origin header. Anvil's
  // worker has no CORS gate at all (nothing but the phone posts to it), and
  // this test is what stops one being added back thoughtlessly.
...
test("endpoint: GET is rejected", async () => {
  const res = await worker.fetch(
    new Request("https://anvil-worker.example.workers.dev/vitals", { method: "GET" }),
    ENV,
  );
